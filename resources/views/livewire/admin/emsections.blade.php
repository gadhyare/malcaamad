...
<div class="col-xs-12 col-sm-12 col-md-8">
        @php
            $serial = ($emplooyesSections->currentpage() - 1) * $emplooyesSections->perpage() + 1;
        @endphp
        {{-- Show all expenses type list --}}
        <x-table :headers="['#',   'الاسم' ,  'الحالة', 'التعديل' , 'الحذف' ]">
            @foreach ($emplooyesSections as $emplooyesSection)
                <tr>
                    <td>{{ $serial++ }}</td>
                    <td>{{ $emplooyesSection->name }}</td>
                    <td>{{ $emplooyesSection->active == 1 ? 'مفعل' : 'غير مفعل' }}</td>
                    <td>
                        <button class="btn bg-success-dark text-white  px-2 py-0 shadow-lg rounded-0" wire:click="updateRec({{ $emplooyesSection->id }})"
                    ...
                    <td>
                        ...
                        wire:click="deleteConfirmation({{ $emplooyesSection->id }})">
                        <small>
