...
<div class="col-xs-12 col-sm-12 col-md-8">
        ...
        <x-table :headers="['#', 'القسم', 'الحالة', 'التعديل', 'الحذف']">
            @foreach ($sections as $section)
                <tr>
                    <td>{{ $serial++ }}</td>
                    <td>{{ $section->name }}</td>
                    <td>{{ $section->active == 1 ? 'مفعل' : 'غير مفعل' }}</td>
                    <td>
                        <button class="btn bg-success-dark text-white  px-2 py-0 shadow-lg rounded-0" wire:click="updateRec({{ $section->id }})"
