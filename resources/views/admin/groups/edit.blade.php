...
@section('content')
    <div class="container my-2 py-2 fw-bold">
        ...
        <div class="card  col-xs-12 col-sm-12 col-md-4 col-xs-3 m-auto ">
            ...
            <div class="card-body ">
                <form action="{{route('groups.edit' , $groups->id)}}" method="post" class="fw-bold">
                    @csrf
                    <div class="form-group my-2">
                        <label for="name">الاسم</label>
                        <input type="text" name="name" id="name" class="form-control bg-white" value="{{$groups->name}}">
                        @error('name')
                    ...
                    <div class="form-group my-2">
                        ...
                        <select name="active" id="active" class="form-select bg-white">
                            <option value="1" {{($groups->active == 1) ? ' selected' : ''}}>مفعل</option>
                            <option value="2" {{($groups->active == 2) ? ' selected' : ''}}>غير مفعل</option>
                        </select>
                        @error('active')
                            <small><i class="text-danger">{{$message}}</i></small>
                        @enderror
